...
def layout(Tab,checklist,checklist_key,tab_num,Image):
    ...
    col_list=[2]
    for i in actions:
        if i not in list(st.session_state.action_dict.values()) and i not in image_actions:
            st.markdown(i)
        ...
        with upper_cols[0]:
            ...
            for num,i in enumerate(image_actions):
                ...
                with upper_cols[num+1]:
                    st.button(i,key=str(i)+checklist_key,on_click=selected(i,checklist_key,cond_dict))
